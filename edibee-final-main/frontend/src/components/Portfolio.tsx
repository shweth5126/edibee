...
function VideoTile({ item }: { item: Item }) {
  ...
  // Autoplay (muted, looped) while on screen on every device; pause when away.
  useEffect(() => {
  ...
  return (
    <figure data-testid={`work-tile-${slug}`} className="group">
      ...
      >
        ...
        <div className="absolute bottom-2.5 right-2.5 flex items-center gap-1.5 opacity-100 transition-opacity duration-300 md:opacity-0 md:group-hover:opacity-100">
